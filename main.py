...
async def on_ready():
    ...
    for member in client.get_server(SERVER_ID).members:
        ...
        async def on_voice_state_update(member_before, member_after):
            ...
            elif member_before.voice.voice_channel and member_before.voice.voice_channel.id == CHANNEL_ID:
                if member_log.join_time:    # 起動時に既にjoinしていた場合は除外
                    diff_second = time.time() - member_log.join_time
                    # message = '{} has connected to {} for {}.'.format(member_log)
